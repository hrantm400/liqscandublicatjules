...
export class TronScannerService implements OnModuleInit {
    ...
    onModuleInit() {
        if (!this.walletAddress) {
            this.logger.warn('TRC20_WALLET_ADDRESS is not set. Custom TRC20 payment scanning will not work.');
        } else {
            this.logger.log(`TronScannerService initialized. Monitoring wallet: ${this.walletAddress}`);
        }
    }
    ...
    async scanForPayments() {
        ...
        try {
            ...
            const toExpire = pendingPayments.filter(p => {
                ...
                const expiresAt = meta?.expiresAt ? new Date(meta.expiresAt) : new Date(p.createdAt.getTime() + 10 * 60000);
            ...
            const activePending = pendingPayments.filter(p => !toExpire.includes(p));
            ...
            for (const payment of activePending) {
                ...
                const match = transactions.find((tx: any) => {
                    ...
                    const isAfterCreation = txTime >= new Date(payment.createdAt.getTime() - 60000); // 1-minute grace period

                    return isIncoming && isAfterCreation && txValueStr === pAmountStr;
                });

                if (match) {
                    this.logger.log(`Found matching transaction for payment ${payment.id}: TxID ${match.transaction_id}`);
